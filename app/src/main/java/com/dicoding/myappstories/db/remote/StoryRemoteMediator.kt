package com.dicoding.myappstories.db.remote
...
class StoryRemoteMediator(
    private val database: StoriesAppDatabase,
    ...
    override suspend fun load(
        ...
        wrapEspressoIdlingResource {
            try {
                val responseData = storiesService.getAllStories(token, page, state.config.pageSize)
                val endOfPaginationReached = responseData.listStory.isEmpty()

                database.withTransaction {
                    if (loadType == LoadType.REFRESH) {
                        database.remoteKeysDao().deleteRemoteKeys()
                        database.getstoriesDao().deleteAllStories()
                    }
                    ...
                    responseData.listStory.forEach { storyResponse ->
                        val stories = Stories(
                            storyResponse.id,
                            storyResponse.name,
                            storyResponse.description,
                            storyResponse.createdAt,
                            storyResponse.photoUrl,
                            storyResponse.lon,
                            storyResponse.lat
                        )

                        database.getstoriesDao().insertStories(stories)
                    }
                }
    ...
    private suspend fun getRemoteKeysForLastItem(state: PagingState<Int, Stories>): RemoteKeys? {
